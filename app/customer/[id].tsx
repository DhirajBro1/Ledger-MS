...
import { calculateNetBalance } from "@/lib/transaction-utils";
...
export default function CustomerDetailsScreen() {
  ...
  const deleteCustomer = useCallback(async () => {
    ...
    await deleteLocalCustomer(customer.clientId);

    if (token) {
      try {
        const apiUrl =
          process.env.EXPO_PUBLIC_API_URL || "http://192.168.100.29:5000";
        const response = await fetch(
          `${apiUrl.replace(/\/$/, "")}/api/customers/${customer.clientId}`,
          {
            method: "DELETE",
            headers: {
        ...
        console.log("Customer delete sync failed:", error);
      }
    }

    router.back();
  }, [customer, router, token]);
  ...
  const confirmDeleteCustomer = () => {
    ...
    Alert.alert(
      "Delete customer?",
      `Remove ${customer.name} and all of their ledger entries?`,
      [
  ...
  const deleteLedgerEntry = async (transaction: Transaction) => {
    if (!customer) {
      return;
    }

    const nextTransactions = customer.transactions.filter(
      (entry) => entry !== transaction,
    );
    const nextDue = calculateNetBalance(nextTransactions);
    ...
    if (token) {
      try {
        ...
        console.log("Auto-sync after transaction delete failed:", error);
      }
    }
  ...
  const confirmDeleteLedgerEntry = (transaction: Transaction) => {
    const normalizedTransaction = normalizeTransactionRecord(transaction);
    const entryLabel =
      normalizedTransaction.description?.trim() ||
      normalizedTransaction.note?.trim() ||
    ...
    Alert.alert("Delete ledger entry?", `Remove ${entryLabel}?`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete",
        style: "destructive",
        onPress: () => {
          void deleteLedgerEntry(transaction);
        },
      },
    ]);
  };
  ...
  const saveTransaction = async () => {
    ...
    if (
      (!parsedCredit || parsedCredit <= 0) &&
      (!parsedDebit || parsedDebit <= 0)
    ) {
    ...
    const nextDue =
      (Number(customer.totalDue) || 0) + transaction.debit - transaction.credit;
    ...
    <ThemedView style={[styles.screen, { backgroundColor: palette.screen }]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        >
          ...
          <Text style={[styles.customerPhone, { color: palette.textMuted }]}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text
              style={[
                styles.summaryValue,
                isCleared ? styles.green : styles.red,
              ]}
            >
          ...
          <Pressable
            style={[
              styles.deleteCustomerButton,
              { borderColor: palette.negative },
            ]}
            onPress={confirmDeleteCustomer}
          >
            <Text
              style={[styles.deleteCustomerText, { color: palette.negative }]}
            >
              Delete customer
            </Text>
          </Pressable>
        ...
        <View style={styles.sectionHeader}>
          ...
          <Text style={[styles.historyCount, { color: palette.textMuted }]}>
        ...
        {ledgerRows.length === 0 ? (
          ...
          >
            <Text
              style={[styles.emptyHistoryText, { color: palette.textMuted }]}
            >
          ...
          >
            ...
            {ledgerRows.map((row) => (
              <View
                key={`${row.sn}-${row.date.toISOString()}`}
                style={[
                  styles.ledgerRow,
                  { borderBottomColor: palette.border },
                ]}
              >
                ...
                <Text
                  style={[
                    styles.ledgerCell,
                    styles.amountCell,
                    row.credit > 0 ? styles.creditText : styles.mutedText,
                    {
                      color:
                        row.credit > 0 ? palette.positive : palette.textMuted,
                    },
                  ]}
                >
                ...
                <Text
                  style={[
                    styles.ledgerCell,
                    styles.amountCell,
                    row.debit > 0 ? styles.debitText : styles.mutedText,
                    {
                      color:
                        row.debit > 0 ? palette.negative : palette.textMuted,
                    },
                  ]}
                >
                ...
                <View style={[styles.ledgerCell, styles.actionCell]}>
                  <Pressable
                    style={[
                      styles.rowDeleteButton,
                      { borderColor: palette.negative },
                    ]}
                    onPress={() => confirmDeleteLedgerEntry(row.transaction)}
                  >
                    <Text
                      style={[
                        styles.rowDeleteButtonText,
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <View style={styles.modalActions}>
              ...
              <Pressable
                style={[styles.modalActionButton, styles.creditButton]}
